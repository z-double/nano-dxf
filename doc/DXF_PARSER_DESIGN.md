...
动态分配：
- 图层记录：从 `0x10` 开始递增（**上限 144 条**，超出与 LTYPE handle 0xA0+ 冲突）
- 用户线型记录：从 `0xA0` 开始递增
- 实体记录：从 `0x100` 开始递增
- 用户块 BLOCK_RECORD：从 `0x200` 开始递增（v1.2 新增）
- 用户块 BLOCK / ENDBLK：从 `0x210` 开始递增（每块占 2 个，v1.2 新增）
- 用户块内实体：从 `0x300` 开始递增（v1.2 新增）
- `$HANDSEED = 0x2000`（覆盖全部动态分配范围）
